...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['party-chief', 'constituency-coordinator', 'data-analyst', 'field-volunteer'];
  ...
  const { data, error, isLoading } = useSWR<JanaSenaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/jana-senas` : null),
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Jana Sena',
      path: isTenantUser ? `/jana-senas/view/${data?.[0]?.id}` : '/jana-senas',
      entity: 'jana_sena',
